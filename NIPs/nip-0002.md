...
```
    ...
    Author: Sergi Canal ( sergi.cg8@gmail.com )
    Discussions-To: #nem-voting channel on NEM2 slack
...
2. [Specification](#specification)
    ...
    2.1. [General Structure](#structure)

    2.2. [Poll Creation](#creation)

    2.3. [Voting](#voting)

    2.4. [Counting Votes](#counting)

    2.5. [Attacks](#attacks)
3. [Motivation](#motivation)
4. [Design Decisions](#design)
5. [Backwards Compativility](#backwards)
...
There is also the possibility of creating a poll with a whitelist where all casted votes weigh equally, and in the future with NIS2, a mosaic vote is going to be implemented where votes count proportional to the amount of a certain mosaic held by the voter.

In this document we provide a Standard specification and a reference implementation for NEM's voting protocol.
...
![poll structure](https://raw.githubusercontent.com/shierve/NEM-voting-specifications/master/structure.png)
...
Voting has existed on nem for a while, and has been used successfully is lots of cases, but at first it existed just as a NanoWallet module, and no standard or reference implementation could be used for using voting outside of it. All services that wanted to use voting needed to implement their own code, which could cause inconsistencies and vulnerabilities.

A standard is needed so that everybody uses the same protocol for voting functions.

## Design Decisions <a name="design"></a>

The implementation uses nem-library for blockchain communication. It integrates seamlessly into nem-library applications both in typescript and node.js. Examples of this are included in the libraries repo.

nem-voting has been tested by humans, used by real projects extensively (Like NanoWallet), and it has a fair amount of automated tests.
...
Previous versions of voting generated a private / public key pair for every account needed to create a poll. But the private key was discarded since it is not needed. Latest versions of the protocol generate just a public key, derived from a hash of the creator's public key. Which provides a huge performance improvement, and allows for poll validation (discussed in the Attacks section of the specification).
...
It is implemented in typescript, using nem-library.

Currently used by the latest version of NanoWallet as well as the block explorer at http://explorer.nemchina.com/ providing big efficiency improvements over previous implementations.

## Backwards compatibility <a name="backwards"></a>
...
Where the first string of the strings array was supposed to correspond to the first address of the addresses array, and it worked for a while, until for some reason the addresses got swapped between different versions of NanoWallet and people sent votes to an option he didn't intend to (The bug has been later identified, but it can not be fixed in old versions of NanoWallet, the only solution is for people to use always the latest version). The structure was changed to be clearer and give no margin for error, by linking the strings directly to their addresses with a map:

`options:{"strings":["yes","no"],"link":{"yes":"TC2BOQO2JVBZMVSFTUILCSQBGOUAZIOCZXTHAP6S","no":"TCKMEQVM32F7BL6IHU2QF4S6JJVXIMDFXN6PXBN6"}}`

Current versions of nem-voting still have compatibility for old structure polls, but due to the unreliability, it is strongly advised to avoid it at all costs and create polls always with the new structure. To make sure polls are created correctly it is important to always use the latest version of NanoWallet to create polls.

## Drawbacks <a name="drawbacks"></a>

Although the POI voting method is working well, the less known whitelist polls are not being used much, and are very limited. You need to select all addresses that you want to allow to vote and the size of the whitelist is limited by NEM's message length limit. For the NEM2 voting update whitelist voting will be replaced entirely by mosaic voting which will allow to just send a mosaic to everyone you want to be able to vote on a poll (with the possibility to give more weight to some accounts), and unlimited polls will be able to be created with the same mosaic, so that the whitelist will be reused easily. This will have real applications without the limitations of a whitelist.

## Alternatives <a name="alternatives"></a>

The technical specification is already written down on https://github.com/shierve/NEM-voting-specifications and the library is already being used, but creating a NIP helps standarize the protocol further.

## References <a name="references"></a>

technical specification with some more minor details: https://github.com/shierve/NEM-voting-specifications#attacks
nem-voting code and documentation: https://github.com/shierve/nem-voting
nem-voting npm module announcement: https://medium.com/nemofficial/voting-update-and-new-npm-module-3ae7132d4025
voting usage analysis: https://medium.com/@shierve/what-we-can-learn-about-the-nem-voting-system-from-the-travel-by-bit-proposal-e160d535d0da

## History <a name="history"></a>

| **Date**      | **Version**   |
| ------------- | ------------- |
| May 11 2018   | Initial Draft |
